...
Reassembler.prototype = {
  reassemble: function (fragment) {
    ...
    if (fragment.last) {
        ...
        var completedat = this.fragments[fragment.messageid].createdat;
        delete this.fragments[fragment.messageid];
        this.statsdclient.increment("errbit.proxy.incomplete.completed");
        var completedException = new Array();
        completedException['data'] = complete;
        completedException['completedAt'] = completedat;
        return complete;
    } else {
      return false;
    }
